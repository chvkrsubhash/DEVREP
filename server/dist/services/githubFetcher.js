...
exports.fetchGitHubDeveloperData = fetchGitHubDeveloperData;
 ...
 */
async function fetchGitHubDeveloperData(username, options = {}) {
    ...
    try {
        const response = await graphqlWithAuth(DEV_SIGNALS_GRAPHQL, {
            login: username,
        ...
        const weeklyCommitCounts = [];
        for (const week of calendarWeeks) {
            const weekSum = (week.contributionDays || []).reduce((sum, day) => sum + (day.contributionCount || 0), 0);
        ...
        const reviewsGiven = new Array(totalReviews).fill(0).map((_, idx) => ({
            id: `rev-${idx}`,
            state: 'APPROVED',
        ...
        const issuesInvolved = new Array(totalIssues).fill(0).map((_, idx) => ({
            id: `iss-${idx}`,
            state: 'CLOSED',
 ...
 */
async function fetchRestGitHubData(username, includePrivate, token) {
